...
def sig_handler(signal, frame):
    ...
    if _verbose:
        ...
        if _follow_file:
            print (colorize_ok(">>> Last Open :{}".format(_last_target_filename)))
        else:
            path = get_path_of(_last_target_filename)
            print (colorize_ok(">>> Last Open :{} in {}".format(_last_target_filename, path)))
    sys.exit(0)
...
def usage():
    ...
    print (' or :  {} [option] DIRECTORY'.format(os.path.basename(sys.argv[0])))
    print ('')
